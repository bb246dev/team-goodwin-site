...
export const HAPN_REFRESH_MS = 30_000;
export const HAPN_PUBLIC_REQUEST_TIMEOUT_MS = 5_000;
export const MAX_LOCATION_BACKOFF_MS = 5 * 60_000;
...
export function versionedLocationUrl(endpoint, nowMs = Date.now(), bucketMs = ACTIVE_REFRESH_MS) {
  const separator = endpoint.includes("?") ? "&" : "?";
  return `${endpoint}${separator}v=${Math.floor(nowMs / bucketMs)}`;
}

async function fetchJson(fetchImpl, endpoint, signal, nowMs) {
  const response = await fetchImpl(versionedLocationUrl(endpoint, nowMs), {
...
export async function loadPublicRaceSnapshot({
  ...
  try {
    const [racePayload, statusPayload] = await Promise.all([
      fetchJson(fetchImpl, PUBLIC_RACES_ENDPOINT, controller.signal, requestTime),
      fetchJson(fetchImpl, PUBLIC_RACE_STATUS_ENDPOINT, controller.signal, requestTime),
    ]);
...
export function createVisibleRefreshPoller({
  load,
  onSuccess,
  onFailure = () => {},
  ...
  maximumBackoffMs = MAX_LOCATION_BACKOFF_MS,
  isEnabled = () => true,
} = {}) {
  let timer = null;
  let inFlight = null;
  ...
  const retryDelay = () => Math.min(intervalMs * (2 ** Math.max(0, failures - 1)), maximumBackoffMs);
  const schedule = (delay = failures ? retryDelay() : intervalMs) => {
    clearTimer();
    if (!enabled()) return;
    timer = setTimeoutImpl(() => { void refresh(); }, delay);
  };
  const refresh = () => {
    if (!enabled()) return Promise.resolve(null);
    if (inFlight) return inFlight;
    inFlight = Promise.resolve().then(load).then((result) => {
      if (onSuccess(result) === false) throw new Error("location_refresh_rejected");
      failures = 0;
      return result;
    }).catch((error) => {
      failures += 1;
  ...
  const visibilityChange = () => {
    clearTimer();
    if (!documentObject?.hidden) void refresh();
  };
  const online = () => {
    clearTimer();
    if (!documentObject?.hidden) void refresh();
...
export function createPublicRacePoller(options = {}) {
  const { onSnapshot, onFailure = () => {}, ...pollerOptions } = options;
  return createVisibleRefreshPoller({
    ...pollerOptions,
    onSuccess(snapshot) {
...
async function fetchRvLocation(fetchImpl, signal) {
  const response = await fetchImpl(versionedLocationUrl(PUBLIC_RV_LOCATION_ENDPOINT), {
    headers: { Accept: "application/json" },
    credentials: "omit",
    cache: "no-store",
    signal,
...
export function createPublicRvPoller({
  ...
  return createVisibleRefreshPoller({
    ...
    onSuccess(result) {
      if (result?.available === true && result.stale === false) onPosition(result.position, result);
      else if (result?.available === true && result.stale === true && onStale) onStale(result);
      else onFallback();
      return true;
    },
  });
