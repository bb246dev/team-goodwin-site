...
Transfers are sequential and per-file. Before any upload, deploy mode re-downloads every existing destination and verifies its exact declared hash. Backend deploy mode also repeats the authenticated runtime-generation baseline check immediately before FTPS preflight. Each affected destination is checked again immediately before upload. Existing destinations require an intact runner-local rollback backup matching the expected remote hash. Each changed file is uploaded separately, downloaded to a temporary non-artifact path, and checked against the approved new/source SHA-256 before the next file begins. If a later file fails, each earlier verified upload is restored in reverse order only when its current remote hash still equals this release's uploaded hash; ambiguous or externally changed destinations are never overwritten automatically. Curl uses bounded retries and `singlecwd`; this small-batch behavior mitigates the Namecheap FTPS 451 behavior seen during larger transfers. There is no delete, mirror, recursive copy, or directory-sync operation.
...
No raw downloaded production content is uploaded as an artifact. Existing files have a runner-local restore source available only during that workflow job. On upload failure, previously verified files are restored automatically in reverse order after a current-hash safety check. A static HTTP/browser/API verification failure invokes the same guarded restore for all byte-verified uploads. Rollback results are metadata-only artifacts. Files newly introduced by a release have a manual-removal instruction; Stage 1 deliberately has no remote deletion operation. Unchanged files are neither uploaded nor restored.

Stage 1 does not provide persistent raw backups after runner teardown. Automatic rollback is limited to pre-existing destinations whose current hash still proves that this release was the last writer. An unverified upload outcome, concurrent external change, or failed restore is recorded but cannot be recovered automatically; because the job is noninteractive and raw backups cannot be persisted as artifacts, a rollback failure may require recovery from a separately maintained server-side backup. When the job finishes, an `always()` cleanup step deletes runner-local backups and FTPS credential temporary directories. Removing a newly created file remains a manual, separately authorized cPanel/FTPS action because automated deletion is prohibited.
